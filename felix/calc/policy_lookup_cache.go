...
func NewPolicyLookupsCache() *PolicyLookupsCache {
	pc := &PolicyLookupsCache{
		nflogPrefixesPolicy:  make(map[model.PolicyKey]set.Set[string]),
		nflogPrefixesProfile: make(map[model.ProfileRulesKey]set.Set[string]),
		nflogPrefixHash:      map[[64]byte]pcRuleID{},
		tierRefs:             map[string]int{},
		ids:                  idalloc.New(),
	}
...
func (pc *PolicyLookupsCache) updatePolicyRulesNFLOGPrefixes(key model.PolicyKey, policy *model.Policy) {
	...
	count, ok := pc.tierRefs["default"]
	if !ok {
		pc.addNFLogPrefixEntry(
			rules.CalculateEndOfTierDropNFLOGPrefixStr(rules.RuleDirIngress, "default"),
			NewRuleID("default", "", "", 0, rules.RuleDirIngress, rules.RuleActionDeny),
		)
		pc.addNFLogPrefixEntry(
			rules.CalculateEndOfTierDropNFLOGPrefixStr(rules.RuleDirEgress, "default"),
			NewRuleID("default", "", "", 0, rules.RuleDirEgress, rules.RuleActionDeny),
		)
		pc.addNFLogPrefixEntry(
			rules.CalculateEndOfTierPassNFLOGPrefixStr(rules.RuleDirIngress, "default"),
			NewRuleID("default", "", "", 0, rules.RuleDirIngress, rules.RuleActionPass),
		)
		pc.addNFLogPrefixEntry(
			rules.CalculateEndOfTierPassNFLOGPrefixStr(rules.RuleDirEgress, "default"),
			NewRuleID("default", "", "", 0, rules.RuleDirEgress, rules.RuleActionPass),
		)
	}
	pc.tierRefs["default"] = count + 1
...
func (pc *PolicyLookupsCache) removePolicyRulesNFLOGPrefixes(key model.PolicyKey) {
	...
	count := pc.tierRefs["default"]
	if count == 1 {
		delete(pc.tierRefs, "default")
...
func deconstructPolicyName(name string) (string, string, string, error) {
	...
	if strings.HasPrefix(name, "knp.default.") {
		return namespace, "default", name, nil
	}
